...
export default function UserTable() {
  ...
  // Handle role change action (Make Admin/User/Verifier)
  const handleRoleAction = async (newRole, userId) => {
    try {
      const token = localStorage.getItem('token');
      if (!token) {
        throw new Error('No authentication token found');
      ...
      const response = await fetch(
        `${BACKEND_URL}/api/user/admin/users/${userId}/role`,
        {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (result.success) {
        ...
        const alertMessages = {
          admin: { message: "👑 User role updated to Admin. Full access granted.", type: "success" },
          verifier: { message: "🔍 User role updated to Verifier. Limited access granted.", type: "success" },
          user: { message: "👤 User role updated to Standard User.", type: "success" }
        };
        
        showAlert(alertMessages[newRole].message, alertMessages[newRole].type);
      } else {
  ...
  const getAccessLevel = (user) => {
    ...
    if (user.role === 'contract') return 'Contract Access';
  ...
  // Get available role actions based on current role
  const getAvailableRoleActions = (currentRole) => {
    const allRoleActions = [
      { value: "admin", label: "Make Admin", icon: Shield, color: "text-purple-700", bgColor: "hover:bg-purple-50" },
      { value: "verifier", label: "Make Verifier", icon: UserCheck, color: "text-blue-700", bgColor: "hover:bg-blue-50" },
      { value: "contract", label: "Make Contract User", icon: FileText, color: "text-green-700", bgColor: "hover:bg-green-50" },
      { value: "user", label: "Make Standard User", icon: Users, color: "text-gray-700", bgColor: "hover:bg-gray-50" },
    ];

    return allRoleActions.filter(action => action.value !== currentRole);
  };
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="p-6 border-b border-gray-200 brand-gradient-panel-soft">
                ...
                {filteredUsers.length > 0 ? (
                  filteredUsers.map((user) => (
                    <tr key={user._id} className="hover:bg-gray-50 transition-colors duration-150">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 brand-gradient-avatar rounded-full flex items-center justify-center text-white font-semibold text-sm">
                            ...
                            {dropdownOpen === user._id && (
                              ...
                              >
                                <div className="py-2">
                                  ...
                                  {getAvailableRoleActions(user.role).map((action) => {
                                    const IconComponent = action.icon;
                                    return (
                                      <button
                                        key={action.value}
                                        onClick={() => handleRoleAction(action.value, user._id)}
                                        className={`flex items-center w-full px-4 py-3 text-sm ${action.color} ${action.bgColor} transition-colors duration-150`}
                                      >
                                        <IconComponent className="w-4 h-4 mr-3" />
                                        {action.label}
